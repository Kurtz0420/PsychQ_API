...
class JokeApiView(
    ...
    def get_queryset(self):
        ...
        if query is not None:
            if '_' in query:  # If query contains '_' it will search build_up part
                ...
                qs = qs.filter(
                    Q(about__icontains=query) | Q(type__iexact=query)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        object_to_update = Joke.objects.filter(id=id_).first()
        ...
        serializer = JokeSerializer(object_to_update, data=request.data, partial=True)
    ...
    def get_object(self):
        ...
        return Joke.objects.get(id=post_id)
...
class JokeRudView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = JokeSerializer
    queryset = Joke.objects.all().order_by('custom_ordering')

    def get_queryset(self):
        return Joke.objects.order_by('custom_ordering')
