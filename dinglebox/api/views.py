...
from dinglebox.api.serializers import ClipSerializer
...
class ClipApiView(
    ...
    def patch(self, request, *args, **kwargs):
        ...
        object_to_update = Clip.objects.filter(id=id_).first()
        ...
        serializer = ClipSerializer(object_to_update, data=request.data, partial=True)
    ...
    def get_object(self):
        ...
        return Clip.objects.get(id=post_id)
...
class ClipRudView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = ClipSerializer
    queryset = Clip.objects.all().order_by('custom_ordering')

    def get_queryset(self):
        return Clip.objects.order_by('custom_ordering')
